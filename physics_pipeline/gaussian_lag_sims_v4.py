...
with open(metadata_filename) as csv_file:
...
for input_dataset in ['cmems','oscar_final','globcurrent15']:
    ...
    elif (input_dataset == 'globcurrent0'):
        parcels_input_files = sorted(glob(data_dir + 'GLOBCURRENT/cmems_obs_mob_glo_phy-cur_my*.nc'))
        filenames = {'U': parcels_input_files,'V': parcels_input_files}
        variables = {'U': 'uo','V': 'vo'} #uo is Ekman+geos field, whereas ugos is geos only
        dimensions = {'U': {'lon':'longitude','lat':'latitude','time':'time','depth':'depth'},
                      'V': {'lon':'longitude','lat':'latitude','time':'time','depth':'depth'}}
        indeces = {'depth':[0]}
        output_dir = data_dir + 'parcels_trajs/gaussian_seeding_sample_sites/GC0_runs/'
    
    elif (input_dataset == 'globcurrent15'):
        parcels_input_files = sorted(glob(data_dir + 'GLOBCURRENT/cmems_obs_mob_glo_phy-cur_my*.nc'))
        filenames = {'U': parcels_input_files,'V': parcels_input_files}
        variables = {'U': 'uo','V': 'vo'} #uo is Ekman+geos field, whereas ugos is geos only
        dimensions = {'U': {'lon':'longitude','lat':'latitude','time':'time','depth':'depth'},
                      'V': {'lon':'longitude','lat':'latitude','time':'time','depth':'depth'}}
        indeces = {'depth':[1]} # sets field to next level for depth
        output_dir = data_dir + 'parcels_trajs/gaussian_seeding_sample_sites/GC15_runs/'
        
    # include depth indices if running in Globcurrent data 
    if (input_dataset == 'globcurrent0') or (input_dataset == 'globcurrent15'):
        fieldset = FieldSet.from_netcdf(filenames, variables, dimensions, indeces, allow_time_extrapolation=True) 
    else:
    ...
    #for m in metadata:
        ...
        # CMEMS & GLOBCURRENT use np.datetime, and OSCAR uses cftime
        ...
        # CMEMS & OSCAR are 0-360 lon, GLOBCURRENT is -180 to 180
        if (input_dataset == 'globcurrent0') or (input_dataset == 'globcurrent15'):
        ...
        elif (input_dataset == 'globcurrent15'): # set depth to 15m
            pset = ParticleSet.from_list(fieldset = fieldset, pclass=JITParticle, 
                                         lon = lon_list, lat = lat_list, 
                                         depth = [15]*len(lon_list), time = [datetime_time]*len(lon_list))
        else:
            pset = ParticleSet.from_list(fieldset = fieldset, pclass=JITParticle, 
                                         lon = lon_list, lat = lat_list, 
